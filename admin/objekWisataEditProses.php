...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newFile && $uploadOk == 1) {
        if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
            ...
            $sql = "UPDATE objekwisata SET nama = ?, deskripsi = ?, foto = ?, updated_at = NOW() WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssi", $nama, $deskripsi, $target_file, $id);
        } else {
        ...
        $sql = "UPDATE objekwisata SET nama = ?, deskripsi = ?, updated_at = NOW() WHERE id = ?";
        $stmt = $conn->prepare($sql);
